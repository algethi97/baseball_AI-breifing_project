...
[![Release](https://img.shields.io/badge/release-v1.1.1-green.svg)](https://github.com/algethi97/baseball_AI-breifing_project/releases)
...
- **원클릭 데이터 내보내기 (CSV)**: 수집된 원본 기사 속성을 완벽히 보존하며 `{키워드}_기사수집_{YYMMDD}_{HHMMSS}.csv` 파일로 즉시 저장 (UTF-8-SIG 인코딩 적용으로 엑셀 한글 깨짐 방지)
...
- **마크다운(`.md`) 자동 저장**:
  - 우측 패널의 `[⚡ 보고서 작성]` 버튼 클릭 시, 화면 렌더링과 동시에 `{키워드}_보고서_{YYMMDD}.md` 파일로 프로젝트 루트에 원스톱 자동 저장
  - 동일 날짜 중복 생성 시 시간(`_{HHMMSS}`)을 덧붙여 기존 보고서 덮어쓰기 방지
- **챗봇 연계 버튼**: 보고서 상단의 `[💬 챗봇에 질문하기]` 버튼으로 원클릭 질의 연계

### 4. ☀️ KBO 전국 11개 구장 실시간 기상정보 대시보드 (Stadium Weather Dashboard)
- **기상청 초단기실황 API 연동**:
...
│       ├── api.py              # JS-Python 브릿지 API (챗봇, 보고서, CSV, 날씨 연동)
