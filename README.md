...
[![Release](https://img.shields.io/badge/release-v1.3.0-green.svg)](https://github.com/algethi97/baseball_AI-breifing_project/releases)
...
네이버 스포츠 뉴스를 실시간으로 대량 수집하고 기사 본문(원문 전체)을 정밀 파싱하며, OpenAI 최신 LLM을 결합하여 심층 브리핑 보고서를 자동 생성합니다. 또한 수집된 기사 원문과 AI 보고서를 관계형 SQLite 데이터베이스로 원클릭 추출하고, 기상청 공공데이터를 통해 전국 11개 프로야구 구장의 실시간 기상 관측 데이터 및 **우천 취소 우려 지수**를 직관적인 대시보드로 제공합니다.
...
- **기사 원문 & 보고서 실시간 지식 연동 (Dynamic Knowledge Injection)**:
  - 수집된 기사 원문 목록(최대 200건)과 AI가 생성한 심층 보고서 전문이 챗봇의 지식 베이스로 실시간 자동 주입
  - *"방금 수집한 기사 중에서 홈런 친 선수 있어?"*, *"보고서의 2번 경기 리뷰를 더 쉽게 설명해줘"*와 같은 심층 후속 Q&A 지원
- **연동 상태 배너 & 맞춤형 퀵 질문**:
  - 참조 중인 기사/보고서 상태를 실시간 배너(`⚪ 일반 모드`, `🔵 기사 원문 참조 중`, `🟢 AI 보고서 연동 완료`)로 시각화
...
### 3. 📝 AI 심층 뉴스 분석 보고서 엔진 (AI In-Depth Report Engine)
- **수집된 기사 원문 100% 종합 심층 분석**: 단순 요약 발췌본이 아닌 수집된 기사의 **본문 전문(Full-Text)**을 기반으로 경기별 주요 승부처, 클러치 상황, 선수 활약상을 AI가 정밀 분석
- **경기 일자 명확 기재**: 경기 리뷰 및 선수 활약상 서술 시 구체적인 경기 일자(예: *"9월 8일 경기에서는..."*)를 문장에 필수 명시
- **마크다운(`.md`) 파일 내보내기**:
  - `[⚡ AI 보고서 작성]` 클릭 시 화면에 즉시 렌더링되며, `[📄 보고서 저장 (.md)]` 버튼 클릭 시 `storage/report/{키워드}_보고서_{YYMMDD}.md` 파일로 깔끔하게 저장
